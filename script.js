...
function render() {
  ...
  filteredTodos.forEach((item) => {
    ...
    contentDiv.className =
      "flex items-center gap-3 flex-1 min-w-0 cursor-pointer select-none";
    ...
    actionDiv.className = "flex items-center gap-1 ml-2 shrink-0";

    const editBtn = document.createElement("button");
    editBtn.type = "button";
    editBtn.title = "Edit aktivitas";
    editBtn.className =
      "opacity-80 sm:opacity-0 sm:group-hover:opacity-100 p-1.5 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 rounded-lg transition-all";
    editBtn.innerHTML =
      '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16.862 3.487a2.25 2.25 0 013.182 3.182L8.25 18.463 4 19.5l1.037-4.25L16.862 3.487z"></path></svg>';
    ...
    deleteBtn.className =
      "opacity-80 sm:opacity-0 sm:group-hover:opacity-100 p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all ml-2 shrink-0";
    deleteBtn.innerHTML =
...
function saveEditedTask(event) {
  ...
  const task = todos.find((item) => item.id === editingTaskId);
...
document
  .getElementById("hide-notification")
